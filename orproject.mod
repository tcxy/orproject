...
maximize profit: # The final profit, equals to the revenue from all products, substract the cost on road, and the rent fee
	sum {i in DEPARTURE, j in DESTINATION, k in CARS, r in PRODUCTS} trans[i, j, k, r] * capacity[k] * revenue[r] - sum{ i in DEPARTURE, j in DESTINATION, k in CARS, r in PRODUCTS} trans[i, j, k, r] * cost[k] - sum {k in CARS} rent[k] * amount[k];
